export default {
  common: {
    serviceName: "1DAY 1OTT",
    loading: "Loading…",
    copied: "Copied!",
    cancel: "Cancel",
    save: "Save",
    edit: "Edit",
    del: "Delete",
    confirmDelete: "Sure?",
    close: "Close",
    change: "Change",
  },
  nav: { home: "Home", calendar: "Calendar", all: "All", myPage: "My Page" },
  footer: {
    legalNav: "Policy links",
    privacy: "Privacy Policy",
    terms: "Terms",
    analyticsSettings: "Analytics settings",
  },
  analytics: {
    title: "Analytics for service improvement",
    description:
      "We use Google Analytics only with your consent to collect pseudonymous usage statistics.",
    learnMore: "Learn more",
    reject: "Reject",
    accept: "Accept",
  },
  seo: {
    profileTitle: "@{{username}}’s watch journal",
    profileDescription:
      "Explore @{{username}}’s movie and TV watch journal with {{count}} logged entries.",
    contentDescription: "Explore viewing logs and reactions for {{title}}.",
  },
  myPage: { back: "← Home" },
  password: {
    title: "Change password",
    desc: "Use at least 8 characters",
    current: "Current password",
    new: "New password",
    confirm: "Confirm new password",
    revokeOtherSessions: "Sign out on other devices",
    change: "Change password",
    changing: "Changing…",
    changed: "Password changed",
    currentRequired: "Enter your current password",
    minLength: "New password must be at least 8 characters",
    mismatch: "New passwords do not match",
    sameAsCurrent: "Choose a password different from your current one",
    incorrect: "Current password is incorrect",
    failed: "Failed to change password",
  },
  passkey: {
    title: "🔑 Passkey",
    desc: "Sign in with device biometrics — no password",
    namePlaceholder: "This device's name (optional)",
    add: "Register this device",
    empty: "No passkeys registered yet",
    registered: "Added {{date}}",
    addFailed: "Registration failed — unsupported browser or cancelled",
    signin: "Sign in with Passkey",
    signinFailed: "Passkey sign-in failed — no registered device or cancelled",
    reauthHint: "Please re-enter your password for security",
    reauthDo: "Confirm & register",
    reauthFailed: "Incorrect password",
  },
  settings: { title: "Settings", language: "Language", theme: "Theme", dark: "Dark", light: "Light" },
  action: {
    addRecord: "+ Log",
    logout: "Log out",
    toggleTheme: "Toggle theme",
    fetch: "Fetch",
    submitRecord: "Log it",
  },
  stat: { streak: "🔥 Current streak", thisMonth: "This month", total: "Total logged" },
  unit: { day_one: "day", day_other: "days", entry_one: "entry", entry_other: "entries" },
  count: {
    entry_one: "{{count}} entry",
    entry_other: "{{count}} entries",
    work_one: "{{count}} title",
    work_other: "{{count}} titles",
    totalEntry_one: "{{count}} entry total",
    totalEntry_other: "{{count}} entries total",
  },
  heatmap: { title: "Activity", hint: "One cell per day · darker = more that day" },
  posters: { title: "Posters", recent: "Recent" },
  byType: { title: "By type", total_one: "{{count}} entry total", total_other: "{{count}} entries total" },
  recent: { title: "Recent logs", empty: "No logs yet. Tap “+ Log” to start." },
  all: {
    title: "All logs",
    search: "Search title, note or date",
    more_one: "Show more ({{count}} left)",
    more_other: "Show more ({{count}} left)",
    empty: "No logs yet. Tap “+ Log” to start.",
    noMatch: "No matching logs.",
  },
  type: {
    all: "All",
    movie: "Movie",
    tv: "Drama",
    variety: "Variety",
    documentary: "Documentary",
    anime: "Anime",
    youtube: "YouTube",
    other: "Custom",
  },
  reaction: { down: "Dislike", up: "Like", love: "Love" },
  genre: {
    "28": "Action", "12": "Adventure", "16": "Animation", "35": "Comedy", "80": "Crime",
    "99": "Documentary", "18": "Drama", "10751": "Family", "14": "Fantasy", "36": "History",
    "27": "Horror", "10402": "Music", "9648": "Mystery", "10749": "Romance", "878": "Sci-Fi",
    "10770": "TV Movie", "53": "Thriller", "10752": "War", "37": "Western",
    "10759": "Action & Adventure", "10762": "Kids", "10763": "News", "10764": "Reality",
    "10765": "Sci-Fi & Fantasy", "10766": "Soap", "10767": "Talk", "10768": "War & Politics",
  },
  landing: {
    badge: "One a day, one green square",
    headline: "Turn what you watch\ninto green squares",
    sub: "Movies, TV, variety, YouTube — log today's watch in one line. Watch your own viewing log fill up green like a GitHub contribution graph.",
    benefitsEyebrow: "Your log, your way — connection when you want it",
    benefitsTitle: "Don't just watch it. Keep it as part of your taste.",
    benefitsSub: "Logging takes seconds, and your growing taste stays easy to see. You decide what remains private and what gets shared.",
    feat1Icon: "🎬",
    feat1Title: "Movies to YouTube",
    feat1Body: "Bring movies, drama, variety, anime and YouTube from every service into one viewing log.",
    feat2Icon: "✍️",
    feat2Title: "Done in one line",
    feat2Body: "Find the title, pick a reaction and leave a quick thought. Logging never gets in the way of watching.",
    feat3Icon: "🌱",
    feat3Title: "See your taste take shape",
    feat3Body: "Green squares, streaks, type stats and posters reveal what you really enjoy over time.",
    feat4Icon: "🔒",
    feat4Title: "You choose what is public",
    feat4Body: "Keep your log private and selectively show only the one-line thoughts you want on your public profile.",
    fediverseEyebrow: "OPEN SOCIAL",
    fediverseTitle: "Your reviews, connected to the fediverse",
    fediverseBody: "Enable federation so people on services like Mastodon can follow your 1DAY 1OTT account and receive new public thoughts in their timeline.",
    fediversePoint1: "Follow from Mastodon and other servers",
    fediversePoint2: "Only thoughts you mark public are delivered",
    fediversePoint3: "Manage federation from your own settings",
    fediverseProfile: "My 1DAY 1OTT",
    fediverseNetwork: "Open social network",
    loginTitle: "Get started",
    loginSub: "30 seconds with email · free",
  },
  preview: {
    eyebrow: "Preview",
    title: "See what using 1DAY 1OTT feels like",
    sub: "From watching your activity grow to logging what you watched today.",
    tabsLabel: "Product screen preview",
    dashboardTab: "My activity",
    recordTab: "Log a watch",
    dashboardCaption: "Your daily watches build into a clear grid and poster collection.",
    recordCaption: "Pick a title, add your reaction and a quick note — done.",
    myPage: "My Page",
    streak: "🔥 Streak",
    thisMonth: "This month",
    total: "Total",
    days: "days",
    entries: "logs",
    heatmapHint: "Your viewing habit, growing day by day",
    recentContent: "Recently watched",
    poster1: "Night\nLog",
    poster2: "Summer\nScenes",
    poster3: "Our\nDay",
    poster4: "The Next\nStory",
    poster5: "Little\nUniverse",
    sampleTitle: "Today's Drama",
    sampleMeta: "Drama · 2026",
    sampleNote: "Already looking forward to the next episode",
  },
  auth: {
    name: "Name",
    email: "Email",
    password: "Password (8+ chars)",
    signup: "Sign up",
    signin: "Sign in",
    toSignup: "Create account",
    toSignin: "Back to sign in",
    failed: "Failed",
    processing: "Processing…",
    emailNotVerified: "Please verify your email. We just sent you a verification email.",
    checkEmailTitle: "Check your inbox",
    verificationSent: "We sent a verification email to {{email}}. Open the link in the email to complete your sign-up.",
    verificationResent: "We sent another verification email to {{email}}.",
    checkSpam: "If you cannot find it, check your spam folder. The link is valid for one hour.",
    resend: "Resend verification email",
    resending: "Sending…",
    resendFailed: "Could not resend the verification email.",
    emailOtpEntry: "Sign in with email verification",
    emailOtpPrompt: "Lost your password or passkey?",
    emailOtpDescription:
      "We will send a sign-in code to the email address on your account.",
    emailOtpRequest: "Send sign-in code",
    emailOtpSent:
      "We sent a sign-in code to {{email}}. Email is only sent for registered accounts.",
    emailOtpCode: "6-digit sign-in code",
    emailOtpSignIn: "Sign in with code",
    emailOtpResend: "Send another code",
    emailOtpBack: "Use another sign-in method",
    emailOtpRequestFailed: "Could not send the sign-in code.",
    emailOtpInvalid: "The sign-in code is incorrect.",
    emailOtpExpired: "The sign-in code has expired. Request a new code.",
    emailOtpTooManyAttempts:
      "Too many incorrect attempts. Request a new code.",
    emailOtpSignInFailed: "Email verification sign-in failed.",
    agreementPrefix: "By signing up, you agree to the",
    agreementAnd: " and ",
    agreementSuffix: ".",
  },
  profileSettings: {
    title: "Profile information",
    publicNotice: "Shown on your public profile and the fediverse",
    avatarAlt: "{{name}}'s profile image",
    chooseImage: "Choose image",
    useDefault: "Use default",
    imageHint: "JPEG, PNG, WebP · up to 5MB",
    name: "Display name",
    bio: "Bio",
    bioPlaceholder: "Tell people about yourself",
    saved: "Profile saved.",
    saveFailed: "Could not save your profile.",
    avatarSaved: "Profile image saved.",
    avatarRemoved: "Switched to the default image.",
    uploadFailed: "Could not upload the image.",
    removeFailed: "Could not remove the image.",
    invalid_image_type: "Only JPEG, PNG, and WebP images are supported.",
    image_too_large: "The image must be 5MB or smaller.",
  },
  share: {
    title: "Public profile",
    publicNote: "/@{{username}} is public",
    usernamePlaceholder: "username",
    public: "Public",
    copyLink: "Copy link",
    openProfile: "Open profile ↗",
    saved: "Saved — reloading…",
    error: "Failed: username taken or invalid (lowercase/digits/_ , 3–20 chars)",
    linkCopied: "Link copied!",
  },
  federation: {
    title: "Enable federation",
    description:
      "Let people follow your profile from Mastodon and receive your new one-line notes.",
    handle: "Fediverse address: {{handle}}",
    warning:
      "Once you enable federation for the first time, your @username cannot be changed even if you disable it later. Posts already delivered to other servers may remain after disabling federation.",
  },
  note: {
    placeholder: "One-line note",
    placeholderOptional: "One-line note (optional)",
    visibility: "Make note public",
    visibilityHint: "Show on your public profile and the fediverse",
    publicTitle: "Notes",
  },
  noteReaction: {
    group: "Note reactions",
    add: "Add reaction",
    picker: "Choose a reaction",
    react: "React with {{emoji}}",
    federatedCount_other: "{{count}} from the fediverse",
    loginToReact: "Sign in to react",
    failed: "Could not save your reaction.",
  },
  modal: {
    title: "What did you watch?",
    recentContents: "Recently watched",
    pickRecent: "Select {{title}}",
    ytPlaceholder: "Paste YouTube URL",
    searchPlaceholder: "Search title (TMDB)",
    searchAllPlaceholder: "Search drama, movie, variety, documentary, and anime",
    manualPlaceholder: "Enter title manually",
    tmdbOff: "TMDB token not set — saving as manual entry for now.",
    date: "Date",
    reactionPrompt: "How was it?",
    ytError: "Couldn’t recognize the YouTube URL.",
    needTitle: "Pick or enter a title.",
    needCategory: "Pick a search result or choose a type first.",
    saveFailed: "Save failed",
  },
  calendar: {
    graphAria: "Monthly log graph",
    prevYear: "Previous year",
    nextYear: "Next year",
    graphTitle: "{{year}} log graph",
    totalTitle: "{{year}} total logs",
    modeIndividual: "Individual",
    modeByType: "By type",
    showAll: "View all logs",
  },
  profile: {
    avatarAlt: "{{name}}'s profile image",
    federationEnabled: "Fediverse account",
    notFoundTitle: "Profile not found",
    notFoundBody: "This user is private or doesn’t exist.",
    toHome: "Go home",
  },
  follow: {
    followers: "Followers",
    followingCount: "Following",
    follow: "Follow",
    following: "Following",
    unfollow: "Unfollow",
    changing: "Updating…",
    loginToFollow: "Sign in to follow",
    loadFailed: "Could not load follow status.",
    changeFailed: "Could not update follow status.",
    listFailed: "Could not load this list.",
    noFollowers: "No followers yet.",
    noFollowing: "Not following anyone yet.",
    privateOmitted: "Private users are not shown in this list.",
    more: "Show more",
    retry: "Try again",
  },
  content: {
    notFoundTitle: "Content not found",
    viewers: "{{count}} watched",
    myRecords: "My records",
    myEmpty: "No records for this yet",
    overview: "Overview",
    channel: "Channel",
    watchOnYouTube: "Watch on YouTube",
    runtime: "{{count}} min",
    seasons: "{{count}} seasons",
    episodes: "{{count}} episodes",
    tmdbScore: "TMDB {{score}}",
    tmdbVotes: "({{count}})",
    statusReturning: "Returning",
    statusEnded: "Ended",
    statusCanceled: "Canceled",
    statusPlanned: "Planned",
    statusProduction: "In production",
  },
  impexport: {
    title: "Import / Export",
    desc: "Upload or download your log as a markdown table. Notes default to private when visibility is blank.",
    download: "Download markdown",
    fileLabel: "Choose markdown file",
    placeholder: "| Date | Title | Type | Reaction | Note | Platform | TMDB ID | Note visibility |\n| 2026-07-15 | Moving | tv | 좋아요 | Great | Disney+ | 95557 | private |",
    preview: "Preview",
    summary: "{{ok}} to add · {{err}} errors · {{dup}} possible dups",
    errorsHead: "Error rows",
    dupsHead: "Possible duplicates (added anyway)",
    matchesHead: "Confirm each TMDB title. Repeated titles only need one selection.",
    matchRows: "rows {{rows}}",
    matchFailed: "TMDB search failed. Check the token and network status.",
    matchNone: "No TMDB results found.",
    matchManual: "Keep as a manual entry without TMDB",
    matchesRequired: "Choose how to link {{count}} more titles.",
    rowLabel: "row {{row}}",
    confirm: "Add {{ok}}",
    cancel: "Cancel",
    done: "Added {{n}}",
    tooMany: "Up to 500 rows at a time.",
    empty: "No valid rows to add.",
    failed: "Import failed.",
    partialFail: "Only {{inserted}} of {{ok}} were added — the rest failed, please try again.",
    retryWarn: "The import request failed. Some rows may have already been added — please preview again before retrying.",
  },
};
